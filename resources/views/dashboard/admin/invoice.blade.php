                                ...
                                @foreach ($invoices as $no => $invoice)
                                    <tr>
                                        ...
                                        @if($invoice->status == 0)
                                            ...
                                            <td>
                                                <a class="btn btn-success" href="{{ route('admin.kiriminvoice', $invoice->id_invoice) }}">Kirim Invoice</a>
                                            </td>
                                        @elseif($invoice->status == 1)
                                            @if($tanggal>=$invoice->tgl_tempo)
                                                <td><div class="badge badge-warning">Melebihi Batas Pembayaran</div></td>
                                                <td>
                                                    <a class="btn btn-danger" href="{{ route('admin.tolakpembayaran', $invoice->id_invoice) }}">Tolak</a>
                                                </td>
                                            @elseif($tanggal<=$invoice->tgl_tempo)
                                                <td><div class="badge badge-warning">Menunggu Pembayaran</div></td>
                                                <td>
                                                    <a class="btn btn-success" href="{{ route('admin.approvepembayaran', $invoice->id_invoice) }}">Invoice Terbayar</a>
                                        ...
                                        @endif
                                        <td>
                                            <a class="btn btn-success" href="{{ route('admin.printinv', $invoice->id_invoice) }}">Cetak</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
